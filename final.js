...
//CLOSE COLLAPSIBLES WHEN CLICKING OUTSIDE NAVBAR
document.addEventListener("click", function(event) {
    var navBar = document.querySelector(".navBAR");
    var infoBoxes = document.querySelectorAll(".hide");
    var isClickInsideNavBar = navBar.contains(event.target);
    
    // Check if click is inside any open info box
    var isClickInsideInfoBox = false;
    ...
    // If click is outside the navBAR and info boxes, close all collapsibles and filter info
    if (!isClickInsideNavBar && !isClickInsideInfoBox) {
        var collapsibles = document.querySelectorAll(".collapsible");
        var filterInfos = document.querySelectorAll(".filterInformation");
        
        collapsibles.forEach(function(collapsible) {
            collapsible.classList.remove("active");
        });
        
        filterInfos.forEach(function(filterInfo) {
            filterInfo.classList.remove("open");
            filterInfo.style.maxHeight = 0;
            filterInfo.style.marginBottom = "0";
        });
        
        // Close all open info boxes with slide-out animation
        infoBoxes.forEach(function(infoBox) {
            if (infoBox.classList.contains("slide-in")) {
...
//INTERACTIVE MAP
    ...
    let scale = 1.5;
    let translateX = -svg.width() * 0.25; // Start at ~25% offset to the right
    let translateY = 0;
...
// POP UP INFORMATION ON CLICK
    $(document).ready(function(){
        ...
        $(".filterDiv").click(function(){
            ...
            var infoBoxMap = {
                ...
                'mustapha': 'Mustapha-Adane-Info',
                'negroarts': 'First-World-Festival-of-Negro-Arts-Info',
                'independence': 'Algerian-Independence-Poster-Info',
                'algerianwar': 'Algerian-Independence-Poster-Info',
            ...
            if (currentlyVisibleBox === infoBoxToShow && $(infoBoxToShow).hasClass('slide-in')) {
                $(infoBoxToShow).removeClass('slide-in').addClass('slide-out');
                setTimeout(function() {
                    $(infoBoxToShow).removeClass('slide-out');
                }, 400);
                currentlyVisibleBox = null;
                return;
            }
            
            // Hide all info boxes first with slide-out animation
            $(".hide.slide-in").removeClass('slide-in').addClass('slide-out');
            setTimeout(function() {
                $(".hide.slide-out").removeClass('slide-out');
            }, 400);
            
            // Show only the matching info box with slide-in animation
            if (infoBoxToShow) {
                $(infoBoxToShow).removeClass('slide-out').addClass('slide-in');
    ...
    $(document).ready(function(){
        ...
        $(document).on("click", "button.closeBtn", function(){
            var $infoBox = $(this).closest(".hide");
            $infoBox.removeClass('slide-in').addClass('slide-out');
            setTimeout(function() {
                $infoBox.removeClass('slide-out');
            }, 400);
        });
    });
